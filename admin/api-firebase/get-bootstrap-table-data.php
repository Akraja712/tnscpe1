...
//admission
if (isset($_GET['table']) && $_GET['table'] == 'admission') {
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $search = $db->escapeString($_GET['search']);
        $where .= "WHERE a.id LIKE '%" . $search . "%' OR a.candidate_name LIKE '%" . $search . "%'";
    }
    
    $sql = "SELECT COUNT(a.id) as total FROM `admission` a LEFT JOIN `category` c ON a.category_id = c.id LEFT JOIN `center` ce ON a.center_id = ce.id " . $where;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row)
        $total = $row['total'];
    ...
    foreach ($res as $row) {
        $operate = ' <a href="edit-admission.php?id=' . $row['id'] . '"><i class="fa fa-edit"></i>Edit</a>';
        $operate .= ' <a class="text text-danger" href="delete-admission.php?id=' . $row['id'] . '"><i class="fa fa-trash"></i>Delete</a>';
        $tempRow['id'] = $row['id'];
        $tempRow['candidate_name'] = $row['candidate_name'];
        $tempRow['fathers_name'] = $row['fathers_name'];
        $tempRow['mothers_name'] = $row['mothers_name'];
        $tempRow['dob'] = $row['dob'];
        ...
        $tempRow['id_proof_type'] = $row['id_proof_type'];
        $tempRow['id_proof_no'] = $row['id_proof_no'];
        $tempRow['center_name'] = $row['center_name'];
        if($row['employeed']==1)
        $tempRow['employeed'] ="<p class='text text-success'>Yes</p>";
        else
      $tempRow['employeed']="<p class='text text-danger'>No</p>";
        $tempRow['operate'] = $operate;
        $rows[] = $tempRow;
    }
    $bulkData['rows'] = $rows;
...
//center
if (isset($_GET['table']) && $_GET['table'] == 'center') {
    ...
    $sql = "SELECT COUNT(`id`) as total FROM `center` ";
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row)
        $total = $row['total'];
   
    $sql = "SELECT * FROM center " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    $db->sql($sql);
    $res = $db->getResult();
...
$res = $db->getResult();
    ...
    foreach ($res as $row) {
        ...
        $tempRow['sgpa'] = $row['sgpa'];
        if($row['status']==1)
        $tempRow['status'] ="<p class='text text-success'>Pass</p>";
        else
      $tempRow['status']="<p class='text text-danger'>Fail</p>";
        $tempRow['operate'] = $operate;
        $rows[] = $tempRow;
    }
    $bulkData['rows'] = $rows;
